...
export function Dashboard() {
  ...
  const visible = useMemo(() => filterIncidents(incidents, query, days), [incidents, query, days]);
  ...
  return <><Header query={query} onQueryChange={setQuery}/><main className="mx-auto max-w-[1540px] px-4 py-8 sm:px-6 lg:px-8"><div className="mb-8 flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Operations command center</p><h1 className="mt-2 text-2xl font-semibold tracking-tight text-white sm:text-3xl">Production incident intelligence</h1><p className="mt-2 max-w-xl text-sm text-slate-400">Live ServiceNow incident posture and investigation context for critical services.</p></div><div className="rounded-xl border border-white/8 bg-white/4 px-3 py-2 text-xs text-slate-400"><span className="mr-2 inline-block h-2 w-2 rounded-full bg-emerald-400"/>Live sources connected</div></div>
